...
class Api
{
    ...
    private $server_name = '';
    private $format = 'json';
    private $sdk_version = 'PHP SDK v1.0.0';
     ...
     * @return array 结果数组
     */
    public function api_msdk($script_name, $params, $qs, $method='post', $protocol='http')
    {

        // add some params: 'version'
        $params['version'] = $this->sdk_version;

        // formed json type params to json-formed params
        $params = json_encode($params);

        $uri = SnsNetwork::makeQueryString($qs);
        $script_name = $script_name . '?' . $uri;
        $url = $protocol . '://' . $this->server_name . $script_name;

        // 通过调用以下方法，可以打印出最终发送到MSDK API服务器的请求参数以及url，默认为注释
